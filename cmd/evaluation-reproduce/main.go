...
func run(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	thresholdPath := flags.String("thresholds", "evaluation/regression/thresholds.json", "versioned regression thresholds")
	...
	if err != nil {
		fmt.Fprintf(stderr, "resolve git commit: %v\n", err)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "build evaluation regression report: %v\n", err)
		return 2
	}
	jsonReport, err := reproduce.JSON(report)
	if err != nil {
		fmt.Fprintf(stderr, "encode evaluation regression report: %v\n", err)
		return 2
	}
	...
	fmt.Fprintf(stdout, "evaluation regression artifacts: %s, %s\n", jsonPath, markdownPath)
	if !report.Regression.Passed {
		for _, check := range report.Regression.Checks {
			if !check.Passed {
				fmt.Fprintf(stderr, "regression: metric=%s baseline=%.12g current=%.12g absolute_delta=%.12g threshold=%.12g\n", check.Metric, check.Baseline, check.Current, check.AbsoluteDelta, check.Threshold)
			}
		}
		return 1
	}
	fmt.Fprintln(stdout, "evaluation regression gate: PASS")
	return 0
}
