...
After spending approval, the authorized operator provides the key through `X08_DASHSCOPE_API_KEY` or `--key-stdin`. The credential is read only after execution confirmations pass, and only the fixed-destination gateway receives it. Do not put the key in command-line arguments, the plan or artifact files. An execution command uses a fresh output directory:
