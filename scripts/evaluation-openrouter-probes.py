...
def main():
    ...
    shared = ''.join(f'Context {i:03d}: This synthetic catalog defines document scope and provenance; context identifiers are framing only and never factual evidence.\n' for i in range(96))
    wiki = []
    ...
    module.save(root/'plan.json',{'pairs':30,'order':'alternating','provider_only':'gmicloud/fp8','cache_benefit_required':False,'steps':wiki,
    ...
    try:
        ...
        module.save(root/'status.json',{'status':'passed','cache_content_invalidation':True,'wiki_pairs':30,'human_review_status':'pending'})
    finally:
        relay.close()
