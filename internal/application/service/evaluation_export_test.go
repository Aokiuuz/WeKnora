...
func evaluationExportQuestionFixture(index int, question string) *types.EvaluationQuestionResultEntity {
	return &types.EvaluationQuestionResultEntity{
		...
		GeneratedText: "+SUM(1,1)", PerSampleMetrics: types.JSON(`{"retrieval_metrics":{"precision":0}}`),
...
func TestPrepareEvaluationJSONExportContainsRecomputableFacts(t *testing.T) {
	svc, _, questions := evaluationExportServiceFixture(
		...
		evaluationExportQuestionFixture(0, "question zero"),
		evaluationExportQuestionFixture(2, "question two"),
	)
	...
	require.Equal(t, 2, document.Questions[1].SampleIndex)
	require.JSONEq(t,
		`[{"rank":1,"pid":-1,"score":0.7,"provenance":"unknown"}]`,
		document.Questions[0].SearchResults.ToString(),
	)
	require.GreaterOrEqual(t, len(questions.limits), 3)
...
func TestPrepareEvaluationCSVExportUsesJSONLiteralsForUserText(t *testing.T) {
	svc, _, _ := evaluationExportServiceFixture(
		...
		evaluationExportQuestionFixture(0, `=HYPERLINK("https://example.test")`),
	)
	prepared, err := svc.prepareEvaluationExport(
...
func TestPrepareEvaluationExportCleansTemporaryFileWhenContextIsCanceled(t *testing.T) {
	tempDir := t.TempDir()
	svc, _, _ := evaluationExportServiceFixture(
		t, types.EvaluationStatueSuccess, evaluationExportQuestionFixture(0, "question"),
	)
	...
	for _, fixture := range []struct {
		...
		t.Run(fixture.name, func(t *testing.T) {
			...
			svc, _, _ := evaluationExportServiceFixture(
				t,
				types.EvaluationStatueSuccess,
				evaluationExportQuestionFixture(0, "first"),
				evaluationExportQuestionFixture(1, "second"),
			)
			_, err := svc.prepareEvaluationExport(
				comparisonServiceContext(), "task-a", "json",
