...
八个引擎各完成 100 页调用，800 / 800 份逐页结果已归档并完成自动评分。

本地服务使用中央处理器（Central Processing Unit，CPU）推理。全部引擎均有逐页调用记录。具有正式 OmniDocBench 标准指标的引擎为 8 / 8 个。人工签名完成 0 / 100 页，优先核验清单仍有 20 页待核验。
...
图 1 同时展示固定分母的执行覆盖与官方断言分数。左图保留失败和缺少完整记录的页面；右图在条形末端标出断言通过数及评分分母。全部引擎均完成 20 页 olmOCR 文档的 125 条官方断言。
...
| PaddleOCR-VL CPU | 100 | 84 | 84 | 82 / 125 | 65.60% | 0 | 436.99 |
| PaddleOCR-VL Cloud | 100 | 99 | 99 | 102 / 125 | 81.60% | 0 | 60.75 |

表中接口错误仍属于已执行页面。官方断言工具异常单独记录，不按模型回答正确处理。各引擎的页数和断言分母一致，接口错误与空输出均纳入相应评分。
...
| PaddleOCR-VL CPU | 0.1527 / 78 | 65.12% / 17 | 0.2019 / 79 | 0.2021 / 9 |
...
MinerU CPU 在本机 100 页中有 98 页产生正文，平均请求耗时 22.21 秒；PaddleOCR-VL CPU 有 84 页产生正文，平均耗时 436.99 秒。这组观测反映当前机器、模型实现和客户端等待限制下的运行表现。

PaddleOCR-VL Cloud 的文字编辑距离为 0.0366，阅读顺序编辑距离为 0.0850。MinerU CPU 的表格页均 TEDS 为 85.71%。MinerU Cloud 通过 104 / 125 条 olmOCR 官方断言。文字、表格和断言分数刻画不同对象，各指标分别比较。

Builtin、MarkItDown、OpenDataLoader 与 WeKnora Cloud 在本轮 80 页图像文档的适配器输出中没有可用正文。后续知识库流程的图片识别与问答效果属于独立测量范围。

## 运行配置与解释范围

本地执行使用 CPU。MinerU 使用 pipeline 后端；PaddleOCR-VL 使用完整布局分析服务。云服务由现有租户配置指定，程序在内存中读取和解密凭据，各引擎只收到本引擎所需配置。云端运行记录未包含服务内部模型与运行库的完整版本；冻结输出可重复评分，重新调用云服务时的版本和结果需要另行核对。
...
| 引擎 | 原始记录类别 | 页面数 |
|---|---|---:|
| MarkItDown | 适配器解析失败，未记录更细原因 | 82 |
| MarkItDown | PDF 转换器键值异常 | 1 |
| OpenDataLoader | Java 命令行程序非零退出 | 2 |
| WeKnora Cloud | 远端解析服务截止时间超限 | 48 |
| WeKnora Cloud | 响应消息超过远端接收上限 | 5 |
| WeKnora Cloud | 连接提前结束 | 3 |
| MinerU CPU | 空输出 | 1 |
| MinerU Cloud | 已完成任务的结果压缩包下载超时 | 4 |
| MinerU Cloud | 空输出 | 1 |
| PaddleOCR-VL CPU | 连接提前结束 | 8 |
| PaddleOCR-VL CPU | 客户端等待超时 | 7 |
| PaddleOCR-VL CPU | 空输出 | 1 |
| PaddleOCR-VL Cloud | 空输出 | 1 |

WeKnora Cloud 的响应消息上限错误涉及远端接收的 4 MiB 消息，输入 PDF 大小与响应消息大小分别记录。MinerU Cloud 的下载超时发生在任务已经完成后的压缩包读取阶段。当前应用包含该下载阶段的有限重试；本实验使用冻结基线程序，实验记录不构成这项修复的线上效果对照。

PaddleOCR-VL CPU 批次有 100 份独立收据，其中 1 份通过中断后的完整性核对补收。批次执行了 15 次服务恢复，已记录的失败页面保持原结果。首个请求提交至五步后处理结束共 12.89 小时，包含中断、服务恢复和评分。

该批次请求耗时中位数为 392.08 秒，采用线性插值的第 95 百分位数为 1000.00 秒。耗时统计纳入全部 100 个已记录请求，包括达到客户端等待上限的请求。独立收据将逐页输入、输出、执行器与基线二进制摘要绑定；原始记录保持完整。

当前主机处理器为 Intel(R) Core(TM) Ultra 7 155H，系统报告物理内存 31.50 GiB，Docker 可用内存 15.37 GiB。两个本地解析容器均限制为 6 个 CPU 核；MinerU 内存上限为 8 GiB，PaddleOCR-VL 为 9 GiB。该资源清单为批次完成后的只读快照，模型配置与逐页运行身份分别保存。
...
评分快照中的 800 份调用记录有 0 份包含逐页二进制摘要，800 份未包含该字段。独立二进制身份与逐页字段覆盖分别记录；现有逐页记录保持其原始内容。源码归档与二进制摘要用于定位运行材料，其验证范围不包含逐位一致的重新编译。

本快照纳入的请求开始时间范围为 `2026-09-11T08:20:28Z` 至 `2026-09-11T22:18:18Z`，使用协调世界时（Coordinated Universal Time，UTC）。生成时间与证据范围分别保存在结构化汇总中，正在执行且没有完整记录的请求不进入当前评分。
...
| PaddleOCR-VL CPU | 100 | 100 | 0 | 0.00 |
...
独立审计核对固定输入、800 份逐页记录及输出、100 份本地长批次收据、八套官方评分回执及其实际分母。核对记录见 [独立证据审计](evidence-audit.json)。程序摘要核对与实际人工内容核验分别记录。
