...
| 本轮外部独立复跑（候选验证） | `25cf5a95` | 同一门禁在干净克隆本地通过；真实退化补丁触发 HTTP 门禁失败并指出召回退化，恢复后复过 |
| Apple M4 工程复现 | `1776aa44` | 该提交的两轮独立运行，后续改动不在其范围 |
| 浏览器 12 项检查 | `db56f0d9` | 当次前端构建的界面行为 |

口径边界：确定性门禁精确比较冻结 golden 指标；真实检索质量另由 20 题固定语料的阈值下限守护，两个数据集约 50 题的小样本实测不能外推为模型整体检索排名。普通对话请求按最佳努力记账，数据库故障窗口内账本可能少于供应商实际调用；评测任务采用严格记账，记账失败即任务失败。嵌入缓存写入或命中事件记录失败不影响向量返回，但会以脱敏告警写入日志（仅租户、模型、条目数与存储错误，不含原文、向量或凭据）。云解析费用未知保持 null；本地接口零费用不含硬件与电费。

界面行为补充：评测工作台应用新筛选条件时，先前打开的任务详情随之失效并被清空，空结果筛选不再并排显示旧详情；进行中的详情请求响应若晚到，也不会恢复已失效的详情。
